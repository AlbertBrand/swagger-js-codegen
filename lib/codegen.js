    ...
    _.forEach(swagger.paths, function(api, path){
        ...
        _.forEach(api, function(op, m){
            ...
            _.forEach(params, function(parameter) {
                ...
                ts.collectRefTypes(parameter, swagger.definitions, data.types);
                parameter.cardinality = parameter.required ? '' : '?';
                method.parameters.push(parameter);
            });

            var okResponse = op.responses['200'] || op.responses['default'];
            method.okResponse.description = okResponse.description;
            method.okResponse.tsType = ts.convertType(okResponse.schema);
            ts.collectRefTypes(okResponse.schema, swagger.definitions, data.types);
...
var getCode = function(opts, type) {
    ...
    if (opts.beautify === undefined || opts.beautify === true) {
        return esformatter.format(source, {
            plugins: ['esformatter-flow']
        });
    } else {
        return source;
    }

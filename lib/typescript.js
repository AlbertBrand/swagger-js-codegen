...
function convertType(swaggerType) {
    ...
    typespec.isAtomic = _.contains(['string', 'number', 'boolean', 'any', 'void'], typespec.tsType);
...
/**
 * Collect full type objects for all referenced types.
 *
 * @param {Object} swaggerType Swagger type definition.
 * @param {Object} swaggerDef Swagger definition root.
 * @param {Array} types Type objects are collected here with name as key.
 */
function collectRefTypes(swaggerType, swaggerDef, types) {
    function visit(swaggerType) {
        if (swaggerType === undefined) {
            return;
        } else if (swaggerType.schema) {
            visit(swaggerType.schema);
        } else if (swaggerType.$ref) {
            var ref = swaggerType.$ref;
            var targetRef = ref.substring(ref.lastIndexOf('/') + 1);
            if(!_.find(types, { name: targetRef })) {
                var targetType = swaggerDef[targetRef];
                if(!targetType) {
                    console.log('Type "' + targetRef + '" not found in definition');
                    return;
                }
                var type = convertType(targetType);
                type.name = targetRef;
                types.unshift(type);
                visit(targetType);
            }
        } else if (swaggerType.type === 'array') {
            visit(swaggerType.items);
        } else if (swaggerType.type === 'object') {
            _.forEach(swaggerType.properties, function (propertyType) {
                visit(propertyType);
            });
        }
    }
...
module.exports.collectRefTypes = collectRefTypes;
